...
const result = await useCases.users.getUser.execute(
  { principalExternalId: 'auth-provider-user-id' },
  {
    ...
    actorUserId: 'auth-provider-user-id'
  }
);
```
...
interface OperationContext {
  ...
  actorUserId: string;      // externalId from auth provider (e.g., Supabase user ID)
...
- **actorUserId**: The `externalId` from your authentication provider (e.g., Supabase user ID, Auth0 user ID). This is NOT the internal database ID.
...
- `actorUserId` from the authenticated user's JWT token (the `externalId`)
