...
const result = await useCases.organizationMemberships.addOrganizationMember.execute(
  {
    ...
    // Membership custom fields (if configured):
...
- `username` (string, required): Username/email of the user to add. [See Username concept](/docs/architecture/username)
- `roleCode` (string, required): Role to assign ('owner', 'admin', or 'member')
- Membership custom fields: Any additional fields defined in your organization membership custom schema

**Output:**
- Returns the created or reactivated `OrganizationMembership` object with custom fields
...
> **Note**: Custom fields are particularly useful for storing member-specific information like department, title, start date, or any other metadata relevant to your organization.
