...
- ✅ **Non-invasive**: Modify responses without changing use case logic
- ✅ **Type-safe**: Full TypeScript support with generics
- ✅ **Fail-safe**: If transformer errors, original response is returned
- ✅ **Context-rich**: Access to request, response, and use case result
- ✅ **Per-endpoint**: Configure transformers for specific handlers

## When to Use Response Transformers

| Use Case | Response Transformer | Use Case Hook |
|----------|---------------------|---------------|
| Remove sensitive fields | ✅ Perfect | ❌ Not recommended |
| Add computed fields | ✅ Perfect | ⚠️ Possible but not ideal |
| Transform field names | ✅ Perfect | ❌ Not recommended |
| Send emails/webhooks | ❌ Not designed for this | ✅ Use `onSuccess` hook |
| Logging/metrics | ❌ Not designed for this | ✅ Use `onFinally` hook |
| Custom validation | ❌ Not designed for this | ✅ Use `afterValidation` hook |
...
```
1. Handler executes use case → Gets domain entity
                               ↓
2. Handler builds base response → Standard API format
                               ↓
3. Transformer runs (if configured) → Modifies response
                               ↓
4. Response sent to client → Transformed or original
```
...
app.listen(3000, () => {
  console.log('🚀 Server running with response transformers');
});
```
