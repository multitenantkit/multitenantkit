...
- Maps `externalId` → `id` and `username` → `email`
...
<Callout type="tip">
**Using Supabase?** You can simplify this significantly with `createSupabaseExpressApp` — see the [Basic Configuration](#with-supabase-recommended) section above.
</Callout>
