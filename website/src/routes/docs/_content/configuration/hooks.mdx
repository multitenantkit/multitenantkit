...
interface OperationContext {
  ...
  actorUserId: string;     // User performing the operation
...
**Executes**: After business logic succeeds
...
**Important**: Wrap side effects in try/catch to prevent aborting on failures.

**Example**:

```typescript
CreateOrganization: {
  afterExecution: async ({ stepResults, shared, context }) => {
