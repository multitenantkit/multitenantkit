import type { IDomainError, IResult, OperationContext } from '../';
 ...
 * ```
 */
export interface IUseCase<TInput, TOutput, TError extends IDomainError = IDomainError> {
    execute(input: TInput, context: OperationContext): Promise<IResult<TOutput, TError>>;
}
