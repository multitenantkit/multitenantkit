...
/**
 * Helper class for UserRepository implementations
 * Provides common utilities for handling UserCustomFieldsConfig
 *
 * Benefits:
...
export class UserRepositoryConfigHelper<TCustomFields = {}> {
     ...
     */
    readonly columnMap: Record<UserBaseField, string>;
    ...
    private readonly config?: UserCustomFieldsConfig<TCustomFields>;

    constructor(
        config?: UserCustomFieldsConfig<TCustomFields>,
        ...
        const generatedColumnMap = this.generateColumnMap(USER_BASE_FIELDS, this.namingStrategy);

        // Apply user overrides from columnMapping
        this.columnMap = {
            ...generatedColumnMap,
            ...config?.columnMapping
        } as Record<UserBaseField, string>;
     ...
     * Example:
     * - getColumnName('id') → 'id'
     * - getColumnName('externalId') → 'external_id' (with snake_case)
     * - getColumnName('externalId') → 'auth_provider_id' (with columnMapping override)
     */
    getColumnName(field: UserBaseField): string {
     ...
     */
    customFieldsToDb(fields: Partial<User & TCustomFields>): Record<string, any> {
     ...
     */
    private inferCustomFieldsToDb(fields: Partial<User & TCustomFields>): Record<string, any> {
    ...
    private transformKey(
        ...
        switch (strategy) {
            ...
            case 'kebab-case':
                // camelCase → kebab-case: firstName → first-name
