...
/**
 * Route definition for archive organization endpoint
 */
export const archiveOrganizationRoute: RouteDefinition = {
    method: 'POST',
    path: '/organizations/:organizationId/archive',
    auth: 'required' // Authentication required for archiving
};

/**
 * Archive organization handler factory
 * Returns a configured handler that uses the injected use cases
 * Archives an organization by setting archivedAt timestamp
 * Only organization owners and admins can archive
 *
 * Generic support:
...
> {
    ...
    return async ({
        ...
        try {
            ...
            // Execute the use case with principalExternalId from authenticated principal
            ...
            if (result.isSuccess) {
                ...
                const baseResponse = {
                    status: 200, // OK - archive successful
...
/**
 * Complete handler package for archive organization
