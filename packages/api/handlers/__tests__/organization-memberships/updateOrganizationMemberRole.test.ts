import type { UseCases } from '@multitenantkit/domain-contracts';
import { createPrincipal } from '@multitenantkit/domain-contracts/shared/auth/Principal';
import {
    BusinessRuleError,
    NotFoundError,
    UnauthorizedError,
    ValidationError
} from '@multitenantkit/domain-contracts/shared/errors';
import { beforeEach, describe, expect, it, vi } from 'vitest';
import {
    makeUpdateOrganizationMemberRoleHandler,
    updateOrganizationMemberRoleHandlerPackage,
    updateOrganizationMemberRoleRoute
} from '../../src/organization-memberships/update-organization-member-role/updateOrganizationMemberRole';

// Helper to create Result-like objects for mocking
const mockResult = {
    ok: <T>(value: T) => ({
        isSuccess: true,
        isFailure: false,
        getValue: () => value,
        getError: () => {
            throw new Error('Cannot get error from success result');
        }
    }),
    fail: <E>(error: E) => ({
        isSuccess: false,
        isFailure: true,
        getValue: () => {
            throw new Error('Cannot get value from failure result');
        },
        getError: () => error
    })
};

describe('UpdateOrganizationMemberRole Handler', () => {
    let mockUseCases: UseCases;
    let mockUpdateOrganizationMemberRoleExecute: ReturnType<typeof vi.fn>;

    beforeEach(() => {
        mockUpdateOrganizationMemberRoleExecute = vi.fn();

        mockUseCases = {
            memberships: {
                updateOrganizationMemberRole: {
                    execute: mockUpdateOrganizationMemberRoleExecute
                }
            }
        } as any;
    });

    describe('Route Configuration', () => {
        it('should have correct route definition', () => {
            expect(updateOrganizationMemberRoleRoute.method).toBe('PUT');
            expect(updateOrganizationMemberRoleRoute.path).toBe(
                '/organizations/:organizationId/members/:userId/role'
            );
            expect(updateOrganizationMemberRoleRoute.auth).toBe('required');
        });
    });

    describe('Happy Path', () => {
        it('should return 200 with updated membership when role is changed', async () => {
            const principalExternalId = '00000000-0000-4000-8000-000000000000';
            const organizationId = '00000000-0000-4000-8000-111111111111';
            const targetUserId = '00000000-0000-4000-8000-222222222222';
            const membershipId = '00000000-0000-4000-8000-333333333333';

            const mockMembership = {
                id: membershipId,
                userId: targetUserId,
                organizationId,
                roleCode: 'admin',
                joinedAt: new Date('2025-01-01T00:00:00.000Z'),
                leftAt: undefined,
                createdAt: new Date('2025-01-01T00:00:00.000Z'),
                updatedAt: new Date('2025-01-15T00:00:00.000Z')
            };

            mockUpdateOrganizationMemberRoleExecute.mockResolvedValue(
                mockResult.ok(mockMembership)
            );

            const handler = makeUpdateOrganizationMemberRoleHandler(mockUseCases);
            const principal = createPrincipal(principalExternalId);

            const response = await handler({
                input: {
                    params: {
                        organizationId,
                        userId: targetUserId
                    },
                    body: {
                        roleCode: 'admin'
                    }
                },
                principal,
                requestId: 'req-123'
            });

            expect(response.status).toBe(200);
            expect((response.body as any).data).toEqual({
                id: membershipId,
                userId: targetUserId,
                organizationId,
                roleCode: 'admin',
                joinedAt: new Date('2025-01-01T00:00:00.000Z'),
                leftAt: undefined,
                createdAt: new Date('2025-01-01T00:00:00.000Z'),
                updatedAt: new Date('2025-01-15T00:00:00.000Z')
            });
            expect(response.headers?.['X-Request-ID']).toBe('req-123');

            expect(mockUpdateOrganizationMemberRoleExecute).toHaveBeenCalledWith(
                {
                    principalExternalId,
                    organizationId,
                    targetUserId,
                    roleCode: 'admin'
                },
                expect.objectContaining({
                    externalId: principalExternalId,
                    auditAction: 'UPDATE_ORGANIZATION_MEMBER_ROLE',
                    organizationId
                })
            );
        });

        it('should include X-Request-ID in response headers', async () => {
            const principalExternalId = '00000000-0000-4000-8000-000000000000';
            const organizationId = '00000000-0000-4000-8000-444444444444';
            const targetUserId = '00000000-0000-4000-8000-555555555555';
            const membershipId = '00000000-0000-4000-8000-666666666666';

            const mockMembership = {
                id: membershipId,
                userId: targetUserId,
                organizationId,
                roleCode: 'admin',
                joinedAt: new Date('2025-01-01T00:00:00.000Z'),
                leftAt: undefined,
                createdAt: new Date('2025-01-01T00:00:00.000Z'),
                updatedAt: new Date('2025-01-15T00:00:00.000Z')
            };

            mockUpdateOrganizationMemberRoleExecute.mockResolvedValue(
                mockResult.ok(mockMembership)
            );

            const handler = makeUpdateOrganizationMemberRoleHandler(mockUseCases);
            const principal = createPrincipal(principalExternalId);

            const response = await handler({
                input: {
                    params: {
                        organizationId,
                        userId: targetUserId
                    },
                    body: {
                        roleCode: 'admin'
                    }
                },
                principal,
                requestId: 'req-header-test'
            });

            expect(response.headers?.['X-Request-ID']).toBe('req-header-test');
        });

        it('should handle promoting member to admin', async () => {
            const principalExternalId = '00000000-0000-4000-8000-000000000000';
            const organizationId = '00000000-0000-4000-8000-777777777777';
            const targetUserId = '00000000-0000-4000-8000-888888888888';
            const membershipId = '00000000-0000-4000-8000-999999999999';

            const mockMembership = {
                id: membershipId,
                userId: targetUserId,
                organizationId,
                roleCode: 'admin',
                joinedAt: new Date('2025-01-01T00:00:00.000Z'),
                leftAt: undefined,
                createdAt: new Date('2025-01-01T00:00:00.000Z'),
                updatedAt: new Date('2025-01-15T00:00:00.000Z')
            };

            mockUpdateOrganizationMemberRoleExecute.mockResolvedValue(
                mockResult.ok(mockMembership)
            );

            const handler = makeUpdateOrganizationMemberRoleHandler(mockUseCases);
            const principal = createPrincipal(principalExternalId);

            const response = await handler({
                input: {
                    params: {
                        organizationId,
                        userId: targetUserId
                    },
                    body: {
                        roleCode: 'admin'
                    }
                },
                principal,
                requestId: 'req-promote'
            });

            expect(response.status).toBe(200);
            expect((response.body as any).data.roleCode).toBe('admin');
        });

        it('should handle demoting admin to member', async () => {
            const principalExternalId = '00000000-0000-4000-8000-000000000000';
            const organizationId = '00000000-0000-4000-8000-101010101010';
            const targetUserId = '00000000-0000-4000-8000-111111111111';
            const membershipId = '00000000-0000-4000-8000-121212121212';

            const mockMembership = {
                id: membershipId,
                userId: targetUserId,
                organizationId,
                roleCode: 'member',
                joinedAt: new Date('2025-01-01T00:00:00.000Z'),
                leftAt: undefined,
                createdAt: new Date('2025-01-01T00:00:00.000Z'),
                updatedAt: new Date('2025-01-15T00:00:00.000Z')
            };

            mockUpdateOrganizationMemberRoleExecute.mockResolvedValue(
                mockResult.ok(mockMembership)
            );

            const handler = makeUpdateOrganizationMemberRoleHandler(mockUseCases);
            const principal = createPrincipal(principalExternalId);

            const response = await handler({
                input: {
                    params: {
                        organizationId,
                        userId: targetUserId
                    },
                    body: {
                        roleCode: 'member'
                    }
                },
                principal,
                requestId: 'req-demote'
            });

            expect(response.status).toBe(200);
            expect((response.body as any).data.roleCode).toBe('member');
        });
    });

    describe('Error Cases - Authentication', () => {
        it('should return 400 when principal is missing', async () => {
            const organizationId = '00000000-0000-4000-8000-131313131313';
            const targetUserId = '00000000-0000-4000-8000-141414141414';
            const handler = makeUpdateOrganizationMemberRoleHandler(mockUseCases);

            const response = await handler({
                input: {
                    params: {
                        organizationId,
                        userId: targetUserId
                    },
                    body: {
                        roleCode: 'admin'
                    }
                },
                principal: undefined,
                requestId: 'req-no-auth'
            });

            expect(response.status).toBe(400);
            // Remove timestamp for comparison
            delete (response.body as any).error.timestamp;
            expect(response.body).toEqual({
                error: {
                    code: 'VALIDATION_ERROR',
                    message: 'Authentication is required for this operation',
                    details: {
                        field: 'principal'
                    },
                    requestId: 'req-no-auth'
                }
            });
            expect(response.headers?.['X-Request-ID']).toBe('req-no-auth');

            expect(mockUpdateOrganizationMemberRoleExecute).not.toHaveBeenCalled();
        });
    });

    describe('Error Cases - Authorization', () => {
        it('should return 401 when user is not authorized to change roles', async () => {
            const principalExternalId = '00000000-0000-4000-8000-000000000000';
            const organizationId = '00000000-0000-4000-8000-151515151515';
            const targetUserId = '00000000-0000-4000-8000-161616161616';
            const unauthorizedError = new UnauthorizedError(
                'Only organization owners and admins can change member roles'
            );
            mockUpdateOrganizationMemberRoleExecute.mockResolvedValue(
                mockResult.fail(unauthorizedError)
            );

            const handler = makeUpdateOrganizationMemberRoleHandler(mockUseCases);
            const principal = createPrincipal(principalExternalId);

            const response = await handler({
                input: {
                    params: {
                        organizationId,
                        userId: targetUserId
                    },
                    body: {
                        roleCode: 'admin'
                    }
                },
                principal,
                requestId: 'req-unauthorized'
            });

            expect(response.status).toBe(401);
            expect(response.body).toMatchObject({
                error: {
                    code: 'UNAUTHORIZED',
                    message: expect.stringContaining(
                        'Only organization owners and admins can change member roles'
                    )
                }
            });
        });
    });

    describe('Error Cases - Validation', () => {
        it('should return 404 when organization not found', async () => {
            const principalExternalId = '00000000-0000-4000-8000-000000000000';
            const organizationId = '00000000-0000-4000-8000-171717171717';
            const targetUserId = '00000000-0000-4000-8000-181818181818';
            const notFoundError = new NotFoundError('Organization', organizationId);
            mockUpdateOrganizationMemberRoleExecute.mockResolvedValue(
                mockResult.fail(notFoundError)
            );

            const handler = makeUpdateOrganizationMemberRoleHandler(mockUseCases);
            const principal = createPrincipal(principalExternalId);

            const response = await handler({
                input: {
                    params: {
                        organizationId,
                        userId: targetUserId
                    },
                    body: {
                        roleCode: 'admin'
                    }
                },
                principal,
                requestId: 'req-not-found'
            });

            expect(response.status).toBe(404);
            // Remove timestamp for comparison
            delete (response.body as any).error.timestamp;
            expect(response.body).toEqual({
                error: {
                    code: 'NOT_FOUND',
                    message: `Organization with identifier '${organizationId}' not found`,
                    details: {
                        resource: 'Organization',
                        identifier: organizationId
                    },
                    requestId: 'req-not-found'
                }
            });
        });

        it('should return 404 when target membership not found', async () => {
            const principalExternalId = '00000000-0000-4000-8000-000000000000';
            const organizationId = '00000000-0000-4000-8000-191919191919';
            const targetUserId = '00000000-0000-4000-8000-202020202020';
            const notFoundError = new NotFoundError(
                'OrganizationMembership',
                `${targetUserId}-${organizationId}`
            );
            mockUpdateOrganizationMemberRoleExecute.mockResolvedValue(
                mockResult.fail(notFoundError)
            );

            const handler = makeUpdateOrganizationMemberRoleHandler(mockUseCases);
            const principal = createPrincipal(principalExternalId);

            const response = await handler({
                input: {
                    params: {
                        organizationId,
                        userId: targetUserId
                    },
                    body: {
                        roleCode: 'admin'
                    }
                },
                principal,
                requestId: 'req-target-not-found'
            });

            expect(response.status).toBe(404);
            expect(response.body).toMatchObject({
                error: {
                    code: 'NOT_FOUND',
                    message: expect.stringContaining('not found')
                }
            });
        });

        it('should return 400 for validation errors', async () => {
            const principalExternalId = '00000000-0000-4000-8000-000000000000';
            const organizationId = '00000000-0000-4000-8000-212121212121';
            const targetUserId = '00000000-0000-4000-8000-222222222221';
            const validationError = new ValidationError('Invalid role code', 'roleCode');
            mockUpdateOrganizationMemberRoleExecute.mockResolvedValue(
                mockResult.fail(validationError)
            );

            const handler = makeUpdateOrganizationMemberRoleHandler(mockUseCases);
            const principal = createPrincipal(principalExternalId);

            const response = await handler({
                input: {
                    params: {
                        organizationId,
                        userId: targetUserId
                    },
                    body: {
                        roleCode: 'invalid' as any
                    }
                },
                principal,
                requestId: 'req-validation'
            });

            expect(response.status).toBe(400);
            // Remove timestamp for comparison
            delete (response.body as any).error.timestamp;
            expect(response.body).toEqual({
                error: {
                    code: 'VALIDATION_ERROR',
                    message: 'Invalid role code',
                    details: {
                        field: 'roleCode'
                    },
                    requestId: 'req-validation'
                }
            });
        });
    });

    describe('Error Cases - Business Rules', () => {
        it('should return 422 when trying to change owner role', async () => {
            const principalExternalId = '00000000-0000-4000-8000-000000000000';
            const organizationId = '00000000-0000-4000-8000-232323232323';
            const targetUserId = '00000000-0000-4000-8000-242424242424';
            const businessError = new BusinessRuleError('Cannot change organization owner role');
            mockUpdateOrganizationMemberRoleExecute.mockResolvedValue(
                mockResult.fail(businessError)
            );

            const handler = makeUpdateOrganizationMemberRoleHandler(mockUseCases);
            const principal = createPrincipal(principalExternalId);

            const response = await handler({
                input: {
                    params: {
                        organizationId,
                        userId: targetUserId
                    },
                    body: {
                        roleCode: 'member'
                    }
                },
                principal,
                requestId: 'req-change-owner'
            });

            expect(response.status).toBe(422);
            expect(response.body).toMatchObject({
                error: {
                    code: 'BUSINESS_RULE_VIOLATION',
                    message: 'Cannot change organization owner role'
                }
            });
        });

        it('should return 422 when member has left', async () => {
            const principalExternalId = '00000000-0000-4000-8000-000000000000';
            const organizationId = '00000000-0000-4000-8000-252525252525';
            const targetUserId = '00000000-0000-4000-8000-262626262626';
            const businessError = new BusinessRuleError(
                'Cannot change role of member who has left'
            );
            mockUpdateOrganizationMemberRoleExecute.mockResolvedValue(
                mockResult.fail(businessError)
            );

            const handler = makeUpdateOrganizationMemberRoleHandler(mockUseCases);
            const principal = createPrincipal(principalExternalId);

            const response = await handler({
                input: {
                    params: {
                        organizationId,
                        userId: targetUserId
                    },
                    body: {
                        roleCode: 'admin'
                    }
                },
                principal,
                requestId: 'req-member-left'
            });

            expect(response.status).toBe(422);
            expect(response.body).toMatchObject({
                error: {
                    code: 'BUSINESS_RULE_VIOLATION',
                    message: expect.stringContaining('left')
                }
            });
        });

        it('should return 422 when role is the same', async () => {
            const principalExternalId = '00000000-0000-4000-8000-000000000000';
            const organizationId = '00000000-0000-4000-8000-272727272727';
            const targetUserId = '00000000-0000-4000-8000-282828282828';
            const businessError = new BusinessRuleError('Member already has this role');
            mockUpdateOrganizationMemberRoleExecute.mockResolvedValue(
                mockResult.fail(businessError)
            );

            const handler = makeUpdateOrganizationMemberRoleHandler(mockUseCases);
            const principal = createPrincipal(principalExternalId);

            const response = await handler({
                input: {
                    params: {
                        organizationId,
                        userId: targetUserId
                    },
                    body: {
                        roleCode: 'member'
                    }
                },
                principal,
                requestId: 'req-same-role'
            });

            expect(response.status).toBe(422);
            expect(response.body).toMatchObject({
                error: {
                    code: 'BUSINESS_RULE_VIOLATION',
                    message: expect.stringContaining('already has')
                }
            });
        });
    });

    describe('Error Cases - Unexpected Errors', () => {
        it('should return 500 for unexpected errors in use case', async () => {
            const principalExternalId = '00000000-0000-4000-8000-000000000000';
            const organizationId = '00000000-0000-4000-8000-292929292929';
            const targetUserId = '00000000-0000-4000-8000-303030303030';
            mockUpdateOrganizationMemberRoleExecute.mockRejectedValue(
                new Error('Database connection lost')
            );

            const handler = makeUpdateOrganizationMemberRoleHandler(mockUseCases);
            const principal = createPrincipal(principalExternalId);

            const response = await handler({
                input: {
                    params: {
                        organizationId,
                        userId: targetUserId
                    },
                    body: {
                        roleCode: 'admin'
                    }
                },
                principal,
                requestId: 'req-unexpected'
            });

            expect(response.status).toBe(500);
            // Remove timestamp for comparison
            delete (response.body as any).error.timestamp;
            expect(response.body).toEqual({
                error: {
                    code: 'INTERNAL_SERVER_ERROR',
                    message: 'An unexpected error occurred',
                    details: {
                        originalMessage: 'Database connection lost'
                    },
                    requestId: 'req-unexpected'
                }
            });
            expect(response.headers?.['X-Request-ID']).toBe('req-unexpected');
        });

        it('should pass through data from domain without re-validation', async () => {
            // Note: We trust the domain layer to return valid data
            const principalExternalId = '00000000-0000-4000-8000-000000000000';
            const organizationId = '00000000-0000-4000-8000-313131313131';
            const targetUserId = '00000000-0000-4000-8000-323232323232';
            const membershipId = '00000000-0000-4000-8000-333333333336';

            const validMembership = {
                id: membershipId,
                userId: targetUserId,
                organizationId,
                roleCode: 'admin',
                joinedAt: new Date('2025-01-01T00:00:00.000Z'),
                leftAt: undefined,
                invitedAt: undefined,
                deletedAt: undefined,
                createdAt: new Date('2025-01-01T00:00:00.000Z'),
                updatedAt: new Date('2025-01-15T00:00:00.000Z')
            };

            mockUpdateOrganizationMemberRoleExecute.mockResolvedValue(
                mockResult.ok(validMembership)
            );

            const handler = makeUpdateOrganizationMemberRoleHandler(mockUseCases);
            const principal = createPrincipal(principalExternalId);

            const response = await handler({
                input: {
                    params: {
                        organizationId,
                        userId: targetUserId
                    },
                    body: {
                        roleCode: 'admin'
                    }
                },
                principal,
                requestId: 'req-parse-error'
            });

            expect(response.status).toBe(200);
            expect((response.body as any).data).toMatchObject({
                id: membershipId,
                userId: targetUserId,
                organizationId,
                roleCode: 'admin'
            });
        });
    });

    describe('Edge Cases', () => {
        it('should handle empty string request ID', async () => {
            const principalExternalId = '00000000-0000-4000-8000-000000000000';
            const organizationId = '00000000-0000-4000-8000-333333333331';
            const targetUserId = '00000000-0000-4000-8000-343434343434';
            const membershipId = '00000000-0000-4000-8000-353535353535';

            const mockMembership = {
                id: membershipId,
                userId: targetUserId,
                organizationId,
                roleCode: 'admin',
                joinedAt: new Date('2025-01-01T00:00:00.000Z'),
                leftAt: undefined,
                createdAt: new Date('2025-01-01T00:00:00.000Z'),
                updatedAt: new Date('2025-01-15T00:00:00.000Z')
            };

            mockUpdateOrganizationMemberRoleExecute.mockResolvedValue(
                mockResult.ok(mockMembership)
            );

            const handler = makeUpdateOrganizationMemberRoleHandler(mockUseCases);
            const principal = createPrincipal(principalExternalId);

            const response = await handler({
                input: {
                    params: {
                        organizationId,
                        userId: targetUserId
                    },
                    body: {
                        roleCode: 'admin'
                    }
                },
                principal,
                requestId: ''
            });

            expect(response.status).toBe(200);
            expect(response.headers?.['X-Request-ID']).toBe('');
        });

        it('should handle membership with null leftAt', async () => {
            const principalExternalId = '00000000-0000-4000-8000-000000000000';
            const organizationId = '00000000-0000-4000-8000-363636363636';
            const targetUserId = '00000000-0000-4000-8000-373737373737';
            const membershipId = '00000000-0000-4000-8000-383838383838';

            const mockMembership = {
                id: membershipId,
                userId: targetUserId,
                organizationId,
                roleCode: 'admin',
                joinedAt: new Date('2025-01-01T00:00:00.000Z'),
                leftAt: undefined,
                createdAt: new Date('2025-01-01T00:00:00.000Z'),
                updatedAt: new Date('2025-01-15T00:00:00.000Z')
            };

            mockUpdateOrganizationMemberRoleExecute.mockResolvedValue(
                mockResult.ok(mockMembership as any)
            );

            const handler = makeUpdateOrganizationMemberRoleHandler(mockUseCases);
            const principal = createPrincipal(principalExternalId);

            const response = await handler({
                input: {
                    params: {
                        organizationId,
                        userId: targetUserId
                    },
                    body: {
                        roleCode: 'admin'
                    }
                },
                principal,
                requestId: 'req-null-left'
            });

            expect(response.status).toBe(200);
            expect((response.body as any).data.leftAt).toBe(undefined);
        });

        it('should handle membership with leftAt date (coverage for ?? branch)', async () => {
            const principalExternalId = '00000000-0000-4000-8000-000000000000';
            const organizationId = '00000000-0000-4000-8000-454545454545';
            const targetUserId = '00000000-0000-4000-8000-464646464646';
            const membershipId = '00000000-0000-4000-8000-474747474747';

            const mockMembership = {
                id: membershipId,
                userId: targetUserId,
                organizationId,
                roleCode: 'admin',
                joinedAt: new Date('2025-01-01T00:00:00.000Z'),
                leftAt: new Date('2025-01-20T00:00:00.000Z'),
                createdAt: new Date('2025-01-01T00:00:00.000Z'),
                updatedAt: new Date('2025-01-15T00:00:00.000Z')
            };

            mockUpdateOrganizationMemberRoleExecute.mockResolvedValue(
                mockResult.ok(mockMembership)
            );

            const handler = makeUpdateOrganizationMemberRoleHandler(mockUseCases);
            const principal = createPrincipal(principalExternalId);

            const response = await handler({
                input: {
                    params: {
                        organizationId,
                        userId: targetUserId
                    },
                    body: {
                        roleCode: 'admin'
                    }
                },
                principal,
                requestId: 'req-left-date'
            });

            expect(response.status).toBe(200);
            expect((response.body as any).data.leftAt).toEqual(
                new Date('2025-01-20T00:00:00.000Z')
            );
        });
    });

    describe('Integration - Operation Context', () => {
        it('should build correct operation context for audit', async () => {
            const principalExternalId = '00000000-0000-4000-8000-000000000000';
            const organizationId = '00000000-0000-4000-8000-393939393939';
            const targetUserId = '00000000-0000-4000-8000-404040404040';
            const membershipId = '00000000-0000-4000-8000-414141414141';

            const mockMembership = {
                id: membershipId,
                userId: targetUserId,
                organizationId,
                roleCode: 'admin',
                joinedAt: new Date('2025-01-01T00:00:00.000Z'),
                leftAt: undefined,
                createdAt: new Date('2025-01-01T00:00:00.000Z'),
                updatedAt: new Date('2025-01-15T00:00:00.000Z')
            };

            mockUpdateOrganizationMemberRoleExecute.mockResolvedValue(
                mockResult.ok(mockMembership)
            );

            const handler = makeUpdateOrganizationMemberRoleHandler(mockUseCases);
            const principal = createPrincipal(principalExternalId);

            await handler({
                input: {
                    params: {
                        organizationId,
                        userId: targetUserId
                    },
                    body: {
                        roleCode: 'admin'
                    }
                },
                principal,
                requestId: 'req-audit'
            });

            expect(mockUpdateOrganizationMemberRoleExecute).toHaveBeenCalledWith(
                {
                    principalExternalId,
                    organizationId,
                    targetUserId,
                    roleCode: 'admin'
                },
                expect.objectContaining({
                    externalId: principalExternalId,
                    auditAction: 'UPDATE_ORGANIZATION_MEMBER_ROLE',
                    organizationId
                })
            );
        });
    });

    describe('Handler Package', () => {
        it('should create complete handler package with correct structure', () => {
            const handlerPackage = updateOrganizationMemberRoleHandlerPackage(mockUseCases);

            expect(handlerPackage).toHaveProperty('route');
            expect(handlerPackage).toHaveProperty('schema');
            expect(handlerPackage).toHaveProperty('handler');

            expect(handlerPackage.route).toEqual(updateOrganizationMemberRoleRoute);
            expect(typeof handlerPackage.handler).toBe('function');
        });

        it('should use UpdateOrganizationMemberRoleRequestSchema', () => {
            const handlerPackage = updateOrganizationMemberRoleHandlerPackage(mockUseCases);

            expect(handlerPackage.schema).toHaveProperty('shape');
            expect(handlerPackage.schema.shape).toHaveProperty('params');
            expect(handlerPackage.schema.shape).toHaveProperty('body');
        });
    });
});
